	...
	<link rel="stylesheet" href="https://www.agc.army.mil/Desktopmodules/SharedLibrary/Plugins/font-awesome5/css/all.min.css?cdv=2504">
	<link rel="stylesheet" href="css/footer-layout.css">
	<div id="footer">
	...
	<script type="text/javascript">
		...
		var speed3 = Number('<cfoutput>#qUserStory.speed3#</cfoutput>')*1000;
		var speed4 = Number('<cfoutput>#qUserStory.speed4#</cfoutput>')*1000;
		var redirectURL = '<cfoutput>#redirectURL#</cfoutput>';
		...
		(function ($) {
			if(redirectURL!='index.cfm')
			{
				"use strict";
				Royal_Preloader.config({
					mode: 'text', // 'number', "text" or "logo"
					text: 'Warfighter Support Directorate',
					timeout: 4,
					showInfo: true,
					opacity: 1,
					background: ['#292929']
		...
		function sendEmail()
		{
			...
			mail.target = "_blank";
			mail.href = "mailto:DLL-WSD-FEEDBACK@usace.army.mil?subject="+$('#subject').val()+"&body=Name: "+$('#name').val()+" %0D%0A%0D%0A"+$('#message').val();
			mail.click();
		}
	</script>
	...
	function Get_down_menus(val){
		...
		if(!visual_flag){
			$(val).find('.icon').removeClass('ion-android-arrow-down');
			$(val).find('.icon').addClass('ion-android-arrow-up');
			//$('.navbar-toggler').css('top', '0px');
		} else {
			$(val).find('.icon').removeClass('ion-android-arrow-up');
			$(val).find('.icon').addClass('ion-android-arrow-down');
			//$('.navbar-toggler').css('top', '56px');
		}
	}

	function openClick(page){
		window.open(page, '_blank');
	}

	function myFunction(val) 
	{
		var x = $(val).parent().find('#myLinks');
	  	if (x.css('display') == "block") {
	    	x.css('display',"none");
			$(val).find('#fa_arrow').removeClass("fas fa-angle-double-up");
			$(val).find('#fa_arrow').addClass("fas fa-angle-double-down");
	  	} else {
	  		x.css('display',"block");
			$(val).find('#fa_arrow').removeClass("fas fa-angle-double-dow");
			$(val).find('#fa_arrow').addClass("fas fa-angle-double-up");
	  	}
	}
	</script>
...
================================================== -->
	<script type="text/javascript">
		$(document).ready(function(){
			if(getCurentFileName()=='index.cfm' || getCurentFileName()==''){
				// if(!getCookie('ShowAnnouncements'))
				// {
				// 	$("#a").css("display", "block");
				// 	$("#b").css("display", "block");
				// }
			...
			//setCookie('ShowAnnouncements', '1', 750);
			...
			console.log(window.localStorage.isMySessionActive);

			if(getCurentFileName()=='index.cfm' || getCurentFileName()==''){
				if(window.localStorage.isMySessionActive=="false"){
					$("#a").css("display", "block");
					$("#b").css("display", "block");
				}
			}
...
<!--- 	<script type='text/javascript' src='js/jquery.min.js'></script> --->
	...
	<script>
		...
		$(window).on('load', function() {
			$('#map_inner').find('svg').attr('width', 1250);
			$('#map_holder').css('left', '3.5%');
