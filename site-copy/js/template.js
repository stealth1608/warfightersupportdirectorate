(function($) { "use strict";
	...
	$(".cls_map").click(function(){
				
		$("#google_map").toggle();

		$(this).toggleClass("close-map show-map")
	});
	$("#additional_links").toggle();
	$(".cls_links").click(function(){
...
//Scrolling	
	...
	function() {  
		...
		$("html").niceScroll({
			...
			cursorwidth:"18px",
			background:"rgb(130, 123, 125)",
			cursorborder:"1px solid rgb(189, 189, 189)",
			cursorborderradius:0,
			autohidemode:false
		});

		$('#ascrail2000').show();
		$('#ascrail2000').css('z-index',1000);
		$('#ascrail2000').find('.nicescroll-cursors').css('width', '20px');
	});
